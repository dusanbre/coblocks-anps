 ...
 */
import { buttonsBlockDeprecated } from "../../js/deprecations/deprecate-anpsblocks-buttons";
...
class CSSGridControl extends Component {
	render() {
		...
		const { layout, fullscreen } = attributes;
											...
											<Button
												...
												onClick={() => {
													...
													if (layoutAttributes[value].wrapper) {
														updateBlockAttributes(
															clientId,
															layoutAttributes[value].wrapper
														);
													}
												}}
											></Button>
				...
				<ToggleControl
					...
					help={
						!!fullscreen
							...
							: __("Toggle to enable fullscreen mode.", "coblocks")
					}
				/>
			</Fragment>
		);
