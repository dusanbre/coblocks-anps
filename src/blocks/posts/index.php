 ...
 */
function anpsblocks_render_posts_block($attributes)
{
	...
	if ('external' === $attributes['postFeedType'] && $attributes['externalRssUrl']) {
		...
		if (is_wp_error($recent_posts)) {
			...
			return '<div class="components-placeholder"><div class="notice notice-error"><strong>' . __('RSS error:', 'anpsblocks') . '</strong> ' . $recent_posts->get_error_message() . '</div></div>';
		}
		...
		if (!$recent_posts->get_item_quantity()) {
			...
			return '<div class="components-placeholder"><div class="notice notice-error">' . __('An error has occurred, which probably means the feed is down. Try again later.', 'anpsblocks') . '</div></div>';
		}

		$recent_posts    = $recent_posts->get_items(0, $attributes['postsToShow']);
		$formatted_posts = anpsblocks_get_rss_post_info($recent_posts);
	} else {

		$recent_posts    = get_posts($args);
		$formatted_posts = anpsblocks_get_post_info($recent_posts);
	}
	...
	return anpsblocks_posts($formatted_posts, $attributes);
}
 ...
 */
function anpsblocks_posts($posts, $attributes)
{
	$class_name  = array();
	$class       = array('wp-block-anpsblocks-posts__inner');
	$block_style = strpos($attributes['className'], 'is-style-stacked') !== false ? 'stacked' : 'horizontal';

	array_push($class_name, 'wp-block-anpsblocks-posts');
	...
	foreach ($posts as $post) {
		...
		$list_items_markup .= '<div class="wp-block-anpsblocks-posts__item">';

		if (null !== $post['thumbnailURL'] && $post['thumbnailURL']) {

			$list_items_markup .= sprintf(
				'<div class="wp-block-anpsblocks-posts__image"><a href="%1$s" class="bg-cover bg-center-center" style="background-image:url(%2$s)"></a></div>',
		...
		$list_items_markup .= sprintf(
			'<div class="wp-block-anpsblocks-posts__content%s">',
		...
		if (isset($attributes['displayPostDate']) && $attributes['displayPostDate'] && 'stacked' === $block_style) {
			...
			$list_items_markup .= sprintf(
				'<time datetime="%1$s" class="wp-block-anpsblocks-posts__date">%2$s</time>',
				esc_url($post['date']),
		...
		if (!$title) {
			...
			$title = _x('(no title)', 'placeholder when a post has no title', 'anpsblocks');
		}
		...
		if (isset($attributes['displayPostContent']) && $attributes['displayPostContent']) {
			...
			$list_items_markup .= sprintf(
				'<div class="wp-block-anpsblocks-posts__excerpt">%1$s</div>',
		...
		if (isset($attributes['displayPostLink']) && $attributes['displayPostLink']) {
			...
			$list_items_markup .= sprintf(
				'<div class="wp-block-anpsblocks-posts__more-link"><a href="%1$s">%2$s</a></div>',
				esc_url($post['postLink']),
 ...
 */
function anpsblocks_get_post_info($posts)
{
 ...
 * @return array Returns posts.
 */
function anpsblocks_get_rss_post_info($posts)
{
 ...
 */
function anpsblocks_register_posts_block()
{
	...
	register_block_type(
		...
		array(
			...
			'render_callback' => 'anpsblocks_render_posts_block',
		)
	);
}
add_action('init', 'anpsblocks_register_posts_block');
