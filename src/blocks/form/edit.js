...
class FormEdit extends Component {
	constructor() {
		...
		this.supportsBlockVariationPicker = this.supportsBlockVariationPicker.bind(
			this
		);
		...
		this.state = {
			...
			template: null,
			subjectValue:
				this.props.attributes.subject || "" === this.props.attributes.subject
					? this.props.attributes.subject
					: anpsblocksBlockData.form.emailSubject
		};
	...
	componentDidMount() {
		apiFetch({ path: "/wp/v2/settings" }).then(res => {
			this.setState({
				...
				isSavedKey:
					res.coblocks_google_recaptcha_site_key === "" ||
					res.coblocks_google_recaptcha_secret_key === ""
						? false
						: true
			});
		});
		...
		if (
			!this.supportsInnerBlocksPicker() &&
	...
	componentDidUpdate(prevProps, prevState) {
		...
		map(TEMPLATE_OPTIONS, elem => {
			if (isEqual(elem.template, this.state.template)) {
				...
				updateBlockAttributes(childBlocks[childBlocks.length - 1].clientId, {
					submitButtonText
				});
			}
		});
		...
		if (
			...
			(prevState.template !== this.state.template &&
				...
				innerBlocks.filter(
					block => block.name === "coblocks/field-submit-button"
				).length < 1) ||
			...
			insertBlock(
				createBlock("coblocks/field-submit-button", {
					submitButtonText: __("Submit", "coblocks")
				}),
				innerBlocks.length,
				clientId,
				false
	...
	saveRecaptchaKey() {
		...
		apiFetch({
			...
			method: "POST",
			data: {
				coblocks_google_recaptcha_site_key: recaptchaSiteKey,
				coblocks_google_recaptcha_secret_key: recaptchaSecretKey
			}
		}).then(() => {
			this.setState({
				isSavedKey: true,
		...
		return (
			<Fragment>
				<TextControl
					...
					value={to || "" === to ? to : anpsblocksBlockData.form.adminEmail}
				...
				<TextControl
					...
					help={
						<Fragment>
							{" "}
							{__(
								"You may use the following tags in the subject field: ",
								"coblocks"
							)}
							<Button isLink onClick={this.appendTagsToSubject}>
								[{__("email", "coblocks")}]
							</Button>
							<Button isLink onClick={this.appendTagsToSubject}>
								[{__("name", "coblocks")}]
			...
			return (
				<Fragment>
					...
					<div className={classes}>
						{this.supportsBlockVariationPicker()
							? this.blockVariationPicker()
							: this.innerBlocksPicker()}
					</div>
				</Fragment>
			);
		}
...
const applyWithSelect = withSelect((select, props) => {
	...
	const { getBlockType, getBlockVariations, getDefaultBlockVariation } = select(
		"core/blocks"
	);
