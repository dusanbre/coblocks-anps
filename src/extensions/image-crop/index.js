...
const supportedBlocks = ["core/image", "core/cover"];
...
const positioningControl = createHigherOrderComponent(BlockEdit => {
	return props => {
		...
		const updateImage = function() {
			...
			fetch(global.ajaxurl, {
				...
				body: [
					...
					`nonce=${anpsblocksBlockData.cropSettingsNonce}`,
				...
				.then(response => {
					...
					if (
						positioningControlData.loadingCount > 0 ||
						positioningControlData.editing
					) {
						return;
					}
		...
		const applyAttributes = function(changeAttributes) {
			const attributeDifference = _.reduce(
				changeAttributes,
			...
			if (attributeDifference) {
				changeAttributes.className =
					removeCropClass(currentAttributes.className) + " is-cropping";
			}
		...
		return (
			<Fragment>
				...
				<InspectorControls>
					<PanelBody
						title={__("Crop settings", "coblocks")}
						initialOpen={false}
					>
						<CropControl
...
addFilter(
	...
	"coblocks/imagePositioning/attributes",
	addPositioningControl
);
