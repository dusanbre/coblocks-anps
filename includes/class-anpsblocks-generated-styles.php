...
class AnpsBlocks_Generated_Styles
{
	...
	{
		wp_add_inline_style('anpsblocks-frontend', $this->styles());
	}
	...
	public function styles()
	{
		...
		if ($post && isset($post->ID)) {
			...
			$meta    = get_post_meta($post->ID, '_anpsblocks_dimensions', true);
			$desktop = array();
			$tablet  = array();
			$mobile  = array();
			$output  = '';
			...
			if ($meta) {
				...
				if (!empty($meta)) {
					...
					foreach ($meta as $id => $block) {
						...
						if (!empty($tablet)) {
							$output .= '@media only screen and (max-width: ' . apply_filters('anpsblocks_tablet_breakpoint', '768px') . ') {';
							$output .= sprintf('.%1$s > div {', esc_attr($id));
						...
						if (!empty($mobile)) {
							$output .= '@media only screen and (max-width: ' . apply_filters('anpsblocks_desktop_breakpoint', '514px') . ') {';
							$output .= sprintf('.%1$s > div {', esc_attr($id));
			...
			$responsive_height = get_post_meta($post->ID, '_anpsblocks_responsive_height', true);
					...
					if (!empty($divider_obj)) {
						foreach ($divider_obj as $divider_element => $divider_el_obj) {
							...
							if ('shapeHeight' === $divider_element && isset($divider_el_obj->heightTablet)) { // @codingStandardsIgnoreLine
								$output     .= sprintf('.%1$s > .wp-block-anpsblocks-shape-divider__svg-wrapper {', esc_attr($divider_key));
								$output .= 'min-height:' . $divider_el_obj->heightTablet . 'px !important'; // @codingStandardsIgnoreLine
								$output     .= '}';
							}
							if ('backgroundHeight' === $divider_element && isset($divider_el_obj->heightTablet)) { // @codingStandardsIgnoreLine
								$output     .= sprintf('.%1$s > .wp-block-anpsblocks-shape-divider__alt-wrapper {', esc_attr($divider_key));
								$output .= 'min-height:' . $divider_el_obj->heightTablet . 'px !important'; // @codingStandardsIgnoreLine
								$output     .= '}';
							}
							$output .= '}';
